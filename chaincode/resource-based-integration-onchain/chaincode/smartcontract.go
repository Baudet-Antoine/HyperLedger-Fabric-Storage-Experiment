...
type BatchResource struct {
	...
	AttachmentContents []string `json:"attachmentContents"`
}
	...
	batches := []BatchResource{
		{
			...
			AttachmentContents: []string{"Contenu de fichier exemple 1", "Contenu de fichier exemple 2"},
		},
	}
...
func (s *SmartContract) CreateBatch(ctx contractapi.TransactionContextInterface, id string, batchID string,
	...
	timestamp string, parentBatchesJSON string, attachmentContentsJSON string) error {
	...
	if attachmentContentsJSON != "" {
		err = json.Unmarshal([]byte(attachmentContentsJSON), &attachmentContents)
		if err != nil {
			return fmt.Errorf("failed to unmarshal attachment contents: %v", err)
		}
	}
...
func (s *SmartContract) AddAttachmentContent(ctx contractapi.TransactionContextInterface, id string, content string) error {
	batch, err := s.ReadBatch(ctx, id)
	if err != nil {
		return err
	}

	batch.AttachmentContents = append(batch.AttachmentContents, content)
	batchJSON, err := json.Marshal(batch)
	if err != nil {
		return err
	}
